...
<h1>Published Rides</h1>
	...
	$sql="select LicenseNumber,Pickup,Destination from driver where Email='".$_POST['Email']."' && verify='verified'";
	$res=$conn->query($sql);
	if($res->num_rows>0){?>
<div class="row">
  <div class="col-sm-6">
    ...
    while($row=$res->fetch_assoc()){
		...
		<div class="card" style="width: 15rem;height: 150px;margin: 50px 50px;padding-top: 0px;">
			<h5 style="background-color: black;color: white;padding-top: 10px;padding-left: 10px;padding-bottom: 10px;">License Number Verified</h5>
  <div class="card-body">
  	<h3><?php echo "From :".$row['Pickup'] ?></h3>
  	<h3><?php echo "To :".$row['Destination'] ?></h3>
